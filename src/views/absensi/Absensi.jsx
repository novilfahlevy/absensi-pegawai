      ...
      .then(response => {
        const absensi = response.data.absensi.map((absensi, i) => ({
          ...
          foto: (
            <Row>
              <Col className="col-6">
                <img key={absensi.id} src={`${process.env.REACT_APP_BASE_URL}storage/attendances_photo/${absensi.foto_absensi_masuk}`} width="100%" height="100%" onClick={() => this.toggleabsenPhotoLightbox(`${process.env.REACT_APP_BASE_URL}storage/attendances_photo/${absensi.foto_absensi_masuk}`)} style={{ cursor: 'pointer' }} alt="Foto Absen Masuk" />
              </Col>
              <Col className="col-6">
                <img key={absensi.id} src={`${process.env.REACT_APP_BASE_URL}storage/attendances_photo/${absensi.foto_absensi_keluar}`} width="100%" height="100%" onClick={() => this.toggleabsenPhotoLightbox(`${process.env.REACT_APP_BASE_URL}storage/attendances_photo/${absensi.foto_absensi_keluar}`)} style={{ cursor: 'pointer' }} alt="Foto Absen Keluar" />
              </Col>
            </Row>
          ),
